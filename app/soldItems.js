...
// Handle file upload
app.post('/soldItemsUpload', upload.single('myFile'), (req, res) => {
  const quantity = req.body.quantity;
  const namePrice = req.body.namePrice;
  const fileName = generatedFileName;

  uploadedItems.push({fileName: fileName, quantity, namePrice});

  console.log('File uploaded', fileName, quantity, namePrice)
  console.log(req.file); // Log information about the uploaded file
  res.send(`File uploaded! Filename:  ${fileName}, Quantity: ${quantity}, price: ${namePrice}`);
});

app.get('/soldUploadedItems', (req, res) => {
